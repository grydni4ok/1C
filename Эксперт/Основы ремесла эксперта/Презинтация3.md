...
`  <log location="C:\Logs\Plan_Postgres" history="2">`  
`		<event>`  
`			<eq property="name" value="dbpostgrs"/>`  
`			<eq property="p:processName" value="pparking"/>`  
`		</event>`
`   <property name="all"/>`  
`	</log>`  
`	<log location="C:\Logs\Plan_MSSQL" history="2">`  
`		<event>`  
`			<eq property="name" value="dbmssql"/>`  
`			<eq property="p:processName" value="sparking"/>`  
`		</event>` 
...
Прописываем путь, для сохранения логов, прописываем события для postgres value="dbpostgrs", и для ms sql value="dbmssql". В конце обязательно <plansql/>.
